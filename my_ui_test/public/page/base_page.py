...
class BasePage(object):
    ...
    def quit_browser(self):
        self.driver.quit()
        self.my_print("Quit this browser.")
    # 打开网页
    def open(self, url):
        """
        open url.
    ...
    def forward(self):
        self.driver.forward()
        self.my_print("Click forward on current page.")

    # 浏览器后退操作
    def back(self):
        self.driver.back()
        self.my_print("Click back on current page.")

    # 隐式等待
    def wait(self, seconds):
        self.driver.implicitly_wait(seconds)
        self.my_print("wait for %d seconds." % seconds)

    # 点击关闭当前窗口
    def close(self):
        try:
            self.driver.close()
            self.my_print("Closing and quit the browser.")
        except NameError as e:
            logger.error("Failed to quit the browser with %s" % e)

    # 保存图片
    def get_img(self,picName):
        """
        在这里我们把file_path这个参数写死，直接保存到我们项目根目录的一个文件夹.\Screenshots下
        """
        rq = time.strftime('%Y%m%d', time.localtime(time.time()))
        file_path = globalparam.img_path + '\\'+rq+"\\"
        if not os.path.exists(file_path):
                os.mkdir(file_path)
        screen_name = file_path+picName + '.png'

        try:
            self.driver.get_screenshot_as_file(screen_name)
            self.my_print("take screenshot and save to folder : %s"%screen_name)
        except NameError as e:
